...
```bash
mkdir -p ~/.claude/rules/studio
cp -r rules/common ~/.claude/rules/studio/
cp -r rules/design ~/.claude/rules/studio/
cp -r rules/product ~/.claude/rules/studio/
cp -r rules/brand ~/.claude/rules/studio/
cp -r rules/copy ~/.claude/rules/studio/
```
...
if a project has its own rules, project rules win. STUDIO's rules are defaults.
